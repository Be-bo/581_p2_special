...
const int PIEZO_PIN = A6; // Piezo output
const int OutPin = 6;
...
void loop() 
{
  ...
  float num = (piezoV - 5) * -100;

  if ((waiting > 10000) && (num == 0)){
    Serial.println("done"); // Print the voltage.
    waiting = 0;
    sing(1);
  ...
  if (num <= 0.9){
    ...
    //alarm();
    //digitalWrite(OutPin, LOW);
    Serial.println(num); // Print the voltage.
  }
  else{
    waiting = 0;
  }
